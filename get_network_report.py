...
OUTPUT_REPORT = f"./reports/TLS_present{present}_KO-{KO_PVAL}_Wk3-{WK3_PVAL}_Wk6-{WK6_PVAL}_FDR-{META_FDR}_diffFilt_report.txt"

NODE_SIGN_FILE = (
    "/nfs7/PHARM/Morgun_Lab/amanda/TkNA_analysis/"
    "TLS_network/combined_TLS_diff.csv"
)
...
starting_after_pval = len(df)

df["PUC_Compliant"] = df.apply(
    lambda row: is_puc(row["Edge Name"], row["KO_median_median"]),
    axis=1
)

removed_puc = (~df["PUC_Compliant"]).sum()
...
removed_meta = starting_after_puc - len(df)


#OUTPUT_EDGES = (
#    f"./filtered_edges_present{present}"
#    f"_KO-{KO_PVAL}_Wk3-{WK3_PVAL}_Wk6-{WK6_PVAL}"
#    f"_META-{META_FDR}_FISHER-{FISHER_FDR}.csv"
#)
...
#print(f"Saved filtered edges: {OUTPUT_EDGES}")
print(f"Edges after meta filtering: {len(df):,}")
...
# =============================================================================
# Save final filtered edge table
# =============================================================================
...
#print(f"Saved filtered edges: {OUTPUT_EDGES}")
print(f"Edges after difference filtering: {len(df):,}")
...
with open(OUTPUT_REPORT, "w") as f:
    ...
    f.write(f"Input edges: {starting_edges:,}\n")
    f.write(f"After ind. p-value filtering:      {after_filt:,}\n")
    f.write(f"Removed PUC-violating edges: {removed_puc:,}\n")
    f.write(f"Meta FDR cutoff:              {META_FDR}\n")
    f.write(f"Fisher FDR cutoff:            {FISHER_FDR}\n")
    f.write(f"Meta-analysis removed:        {removed_meta:,}\n")
    f.write(f"Remaining edges: {edge_count:,}\n")
